...
给同一系列的文章打上同一个标签，标签名以 `系列·` 开头即可（也兼容 `系列:` / `系列-`），例如 `系列·从源码看懂 Codex`。

| 位置                       | 表现                                                                                     |
| -------------------------- | ---------------------------------------------------------------------------------------- |
| 文章详情页                 | 头部出现「系列 · 名称 · 第 N 篇 / 共 M 篇」胶囊，点击进入系列页                          |
| 分类页（如「技术分享」）   | 同系列文章收成一张系列卡（系列名 + 共 N 篇 + 最近更新），点它进系列页；其余文章照常逐条列出 |
| 系列页（该标签的标签归档） | 页头是系列名与篇数，文章按发布时间升序、卡片带序号                                       |
| 其它列表（文章列表 / 归档 / 标签集合） | 不折叠，按原样展示                                                             |

- **加入系列**：新文章带上同一个 `系列·XXX` 标签即可，建议从已有文章复制该标签（标签名逐字一致，它决定系列身份）；不打这个标签的文章就是普通文章，不会被收进系列；
- **别名（slug）注意**：不要以 `/` 开头、不要带空格或制表符。Halo 会用别名拼永久链接，`/` 开头的别名会生成 `/archives//xxx` 或 `%2F` 形式，被请求防火墙拦成 400，文章点不进去；出问题可用 `scripts/fix-post-slugs.mjs` 批量清理；
- **退出系列**：把该标签从文章上移除并更新发布，系列卡上的篇数、系列页列表、其它文章的「第 N 篇 / 共 M 篇」都会自动重算；
- 系列内顺序是**发布时间升序**，按顺序发布即可（置顶与优先级不会打乱它）；序号由 CSS 计数器生成，无限滚动翻到下一页会接着数；
- 展示时会剥掉 `系列·` 前缀；系列标签不再出现在文章头部的普通标签行里；
- 卡片标题里自带的「第X篇：」在系列页与系列卡上会被自动剥掉（标题本身与列表页不受影响）；
- 未发布的文章不会进入系列；系列只有 1 篇时也照常显示。
